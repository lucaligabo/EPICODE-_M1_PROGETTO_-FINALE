# PROGETTO FINALE

# Progetto – Analisi di Vendite in una Catena di Negozi

# Scenario reale
# Una catena di negozi di elettronica vuole analizzare i dati delle vendite per migliorare la gestione e capire l’andamento del mercato. 
# I dati vengono raccolti giornalmente e comprendono informazioni su prodotti, quantità vendute, prezzo, incassi e negozi.

# Si richiede di sviluppare un programma in Python che utilizzi:
# -NumPy per elaborazioni numeriche veloci
# -Pandas per la gestione e analisi di dataset
# -Matplotlib per la visualizzazione dei dati in grafici

import random
import os
from datetime import datetime,timedelta
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt

# Cartella unica per tutti i file generati dal programma
OUTPUT_DIR = "output"
os.makedirs(OUTPUT_DIR, exist_ok=True)

# (Opzionale)Formatta i numeri decimali di Pandas a due cifre nell'output
# Questa impostazione riguarda solo la visualizzazione, non i calcoli
pd.options.display.float_format = "{:.2f}".format

# Impostazione del seed per la riproducibilità dei risultati
random.seed(42)
np.random.seed(42)

# ----------------------------------------
# PARTE 1: Creazione del dataset di base
# ----------------------------------------
negozi = ["Verona", "Roma", "Milano", "Napoli", "Torino", "Bologna"]
prodotti = ["Smartphone", "PC Gaming", "Air Pods", "TV OLed", "SoundBar", "Monitor 144Hz-2K"]
prezzi = {
    "Smartphone": 199.99,
    "PC Gaming": 1800.00,
    "Air Pods": 45.00,
    "TV OLed": 1500.00,
    "SoundBar": 200.00,
    "Monitor 144Hz-2K": 350.00,
}

vendite = {
    "data": [],
    "negozio": [],
    "prodotto": [],
    "quantita": [],
}

# Genera 50 vendite casuali
for _ in range(50):
    prodotto = random.choice(prodotti)
    vendite["data"].append((datetime(2026, 1, 1) + timedelta(days=random.randint(0, 365))).strftime("%Y-%m-%d"))
    vendite["negozio"].append(random.choice(negozi))
    vendite["prodotto"].append(prodotto)
    vendite["quantita"].append(random.randint(1, 20))

# Salvataggio iniziale del file CSV
df_iniziale = pd.DataFrame(vendite)
df_iniziale["prezzo_unitario"] = df_iniziale["prodotto"].map(prezzi)
df_iniziale.to_csv(os.path.join(OUTPUT_DIR, "vendite.csv"), index=False)
print("File 'vendite.csv' creato e salvato con successo!")

# ----------------------------------------
# PARTE 2: Importazione con Pandas
# ----------------------------------------

# Importa il dataset appena creato
df = pd.read_csv(os.path.join(OUTPUT_DIR, "vendite.csv"))
# Visualizza le prime righe, le dimensioni e le informazioni del dataset
print("\nStampa delle prime 5 righe (head):")
print(df.head())
print(f"\nDimensioni dataset (righe, colonne): {df.shape}")
print("\nInformazioni generali (info):")
df.info()

# ----------------------------------------
# PARTE 3: Elaborazioni con Pandas
# ----------------------------------------

# Calcola l'incasso di ogni vendita
df["incasso"] = df["quantita"] * df["prezzo_unitario"]

# Salva il dataset aggiornato con la colonna incasso
df.to_csv(os.path.join(OUTPUT_DIR, "vendite.csv"), index=False)
print("\nAggiunta colonna INCASSO. File csv, salvato correttamente")

# Cìalcola incasso totale della catena
incasso_totale = df["incasso"].sum()
print(f"\nIncasso totale della catena: € {incasso_totale:,.2f}")

# Calcola incasso medio per negozio
incasso_medio_negozio = df.groupby("negozio")["incasso"].sum().mean()
print(f"Incasso medio per negozio: € {incasso_medio_negozio:,.2f}")

# Trova i top 3 prodotti venduti
prodotti_top3 = (
    df.groupby("prodotto")["quantita"].sum().sort_values(ascending=False).head(3)
)
print("\nI 3 prodotti più venduti (quantità):")
print(prodotti_top3)

# Ragruppa incasso medio per prodotto e negozio 
raggruppamento = df.groupby(["negozio", "prodotto"])["incasso"].mean()
print("\nIncasso medio per Negozio e Prodotto (Anteprima):")
print(raggruppamento.head(5))

# ----------------------------------------
# PARTE 4: Elaborazioni numeriche con NumPy
# ----------------------------------------

# Estrae la colonna quantità come array NumPy
qta = df["quantita"].to_numpy()

# Calcola media, minimo, massimo e deviazione standard
media = np.mean(qta)
print (f"\nQuantità media venduta: {media:.2f} | Minimo: {np.min(qta)} | Massimo: {np.max(qta)} | Dev. Standard: {np.std(qta):.2f}")

# Calcola la percentuale di vendite sopra la media
vendite_sopra_media = (len(qta[qta > media]) / len(qta))*100
print (f"La percentuale di vendite sopra la media è: {vendite_sopra_media:.2f}%")

# Crea un array 2D con quantità e prezzo unitario
qta_prezzo = df[["quantita","prezzo_unitario"]].to_numpy()

# Calcola l'incasso di ogni riga usando NumPy
incasso_calcolato = qta_prezzo[:,0] * qta_prezzo[:,1]

# Confronta gli incassi calcolati con quelli presenti nel DataFrame
controllo_incassi = np.allclose(incasso_calcolato,df["incasso"].to_numpy())
print(f"Verifica incassi (Pandas Vs Numpy): {controllo_incassi}")

# ----------------------------------------
# PARTE 5: Visualizzazioni con Matplotlib
# ----------------------------------------

# 1. Grafico a barre: incasso totale per ogni negozio
inc_neg = df.groupby("negozio")["incasso"].sum()
plt.figure(figsize=(8, 4))
plt.bar(inc_neg.index, inc_neg.values, color="skyblue", edgecolor="black")
plt.xticks(rotation=45, ha="right")
plt.ylabel("Incasso (€)")
plt.title("Incasso Totale per Negozio")
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, "grafico_barre_negozi.png"))
plt.show()


# 2. Grafico a torta: percentuali nella torta + legenda con valore in € e percentuale
inc_prd = df.groupby("prodotto")["incasso"].sum()
labels = inc_prd.index.tolist()
values = inc_prd.values
total = values.sum()
fig, ax = plt.subplots(figsize=(8, 6))
# Mostra solo le percentuali all'interno delle fette
wedges, texts, autotexts = ax.pie(
    values,
    labels=None,            # Nessuna etichetta direttamente sulla torta
    autopct="%1.1f%%",      # Percentuale dentro ogni fetta
    startangle=90,
    wedgeprops={"edgecolor": "white"}
)
ax.set_title("Percentuale Incassi per Prodotto")
ax.axis("equal")  # mantiene la torta circolare
# Costruisce le etichette della legenda con valore e percentuale
percent = values / total * 100
legend_labels = [f"{lab}: € {val:,.0f} ({pct:.1f}%)" for lab, val, pct in zip(labels, values, percent)]
# Mostra la legenda a destra, sincronizzata con i colori della torta
ax.legend(wedges, legend_labels, title="Prodotto", loc="center left", bbox_to_anchor=(1, 0.5), fontsize=9)
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, "grafico_torta_prodotti.png"), bbox_inches="tight", dpi=300)
plt.show()


# 3. Grafico a linee: andamento giornaliero degli incassi totali della catena
plt.figure(figsize=(10, 4))
# Visualizza l'andamento giornaliero degli incassi
df.groupby("data")["incasso"].sum().plot(kind="line", marker="o", color="green")
plt.title("Andamento Giornaliero degli Incassi Totali")
plt.xlabel("Data")
plt.ylabel("Incasso Totale (€)")
plt.xticks(rotation=45, ha="right")
plt.grid(True, linestyle="--", alpha=0.6)
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, "grafico_linee_andamento.png"))
plt.show()


# ----------------------------------------
# PARTE 6: Analisi avanzata
# ----------------------------------------

# Definisce le categorie associate ai prodotti
product_to_category = {
    "Smartphone": "Telefonia",
    "PC Gaming": "Informatica",
    "Air Pods": "Audio",
    "TV OLed": "TV",
    "SoundBar": "Audio",
    "Monitor 144Hz-2K": "Informatica",
}

# Crea la colonna categoria nel DataFrame
df["categoria"] = df["prodotto"].map(product_to_category).fillna("Altro")

# Calcola incasso totale e quantità media per categoria
riepilogo_categoria = df.groupby("categoria").agg(
    Incasso_totale = ("incasso", "sum"),
    Quantita_media  = ("quantita", "mean")
).reset_index()

# Arrotonda la quantità media a due decimali per una migliore leggibilità
riepilogo_categoria["Quantita_media"] = riepilogo_categoria["Quantita_media"].round(2)

# Aggiunge le statistiche aggregate al DataFrame originale
df = df.merge(riepilogo_categoria, on="categoria", how="left")

# Controlla che le nuove colonne siano presenti
print("\n", df.head())

# (Opzionale) Mostra i nomi delle colonne del DataFrame aggiornato
print(df.columns.tolist())

# Salva il DataFrame aggiornato
df.to_csv(os.path.join(OUTPUT_DIR, "vendite_analizzate.csv"), index=False)

# Salva anche il riepilogo per categoria in un file separato
riepilogo_categoria.to_csv(os.path.join(OUTPUT_DIR, "vendite_per_categoria.csv"), index=False)


# ----------------------------------------
# PARTE 7: Estensione facoltativa
# ----------------------------------------

# Grafico combinato: incasso medio per categoria e quantità media venduta
fig, ax1 = plt.subplots(figsize=(8, 5))

# Calcola le metriche per categoria
categoria_stats = df.groupby("categoria").agg(
    Incasso_Medio=("incasso", "mean"),
    Quantita_Media=("quantita", "mean")
).sort_values("Incasso_Medio", ascending=False)

# Disegna le barre dell'incasso medio
categoria_stats["Incasso_Medio"].plot(
    kind="bar", ax=ax1, color="orange", alpha=0.7, label="Incasso medio (€)"
)
ax1.set_ylabel("Incasso medio (€)", color="orange")
ax1.tick_params(axis="y", labelcolor="orange")

# Crea l'asse secondario per la quantità media
ax2 = ax1.twinx()
x = np.arange(len(categoria_stats))
ax2.plot(x, categoria_stats["Quantita_Media"].values, color="blue", marker="s",
         linewidth=2, label="Quantità media")
ax2.set_ylabel("Quantità media venduta", color="blue")
ax2.tick_params(axis="y", labelcolor="blue")

# Allinea i valori dell'asse X alle etichette delle categorie
ax1.set_xticks(x)
ax1.set_xticklabels(categoria_stats.index, rotation=45, ha="right")

plt.title("Incasso medio per categoria e quantità media venduta")
fig.tight_layout()
fig.savefig(os.path.join(OUTPUT_DIR, "grafico_combinato_categoria.png"), bbox_inches="tight", dpi=300)  # salva prima di show
plt.show()


# Funzione top_n_prodotti: restituisce i primi n prodotti per incasso totale
def top_n_prodotti(n, df=df):
    # Raggruppa i prodotti e li ordina per incasso totale decrescente
    return (
        df.groupby("prodotto")["incasso"]
          .sum()
          .sort_values(ascending=False)
          .head(n)
    )

# Esempio di utilizzo della funzione
print("\nTop 3 prodotti per incasso totale:")
print(top_n_prodotti(3))